...
const AdminDashboard = () => {
  ...
  >
    <Box sx={{ mb: 3 }}>
      ...
      <KeyMetricsCard refreshVersion={refreshVersion} />
    </Box>

    <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', lg: 'minmax(0, 2fr) minmax(310px, 1fr)' }, gap: 2.5, alignItems: 'start' }}>
      <Box sx={{ display: 'grid', gap: 2.5 }}>
        <Section title="Appointment pulse" subtitle="How bookings are moving through the studio"><AppointmentOverview refreshVersion={refreshVersion} /></Section>
        <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', xl: '1fr 1fr' }, gap: 2.5 }}>
          <Section title="Booking trend" subtitle="Scheduled dates from the last 30 days onward"><AppointmentsChart refreshVersion={refreshVersion} /></Section>
          <Section title="Artist workload" subtitle="Appointment volume by artist"><ArtistPerformanceChart refreshVersion={refreshVersion} /></Section>
        </Box>
      </Box>
      <Section
      ...
      >
        <NotificationsPanel onAppointmentChange={() => setRefreshVersion(value => value + 1)} />
      </Section>
    </Box>
  </PageContainer>
  );
